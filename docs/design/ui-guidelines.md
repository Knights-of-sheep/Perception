...
2. **数据为先**：产品是曲线/结构数据查看器，视图区永远是视觉主角。
...
7. **Dock 面板窗口控制**：面板标题栏自带窗口控制按钮，无系统标题栏。停靠态 = `最大化（分离并最大化）+ 分离 + 关闭`；浮动态（无边框）= `最小化 + 最大化/还原 + 恢复嵌入 + 关闭`，标题栏拖动移动、双击切换最大化/还原、右下角手柄调整大小。按钮图标统一 `makeWinBtnIcon` 16px 矢量绘制。
