...
// 新的回调函数，用于更新第二次切割的平面
class SecondPlaneCallback : public vtkCommand
{
public:
    static SecondPlaneCallback *New()
    {
        return new SecondPlaneCallback;
    }
    ...
    void Execute(vtkObject *caller, unsigned long, void*) override
    {
        ...
        vtkPlane *plane = vtkPlane::New();
        planeWidget->GetPlane(plane);
        secondCutter->SetCutFunction(plane);
        secondCutter->Update();
        plane->Delete();
    }

    vtkCutter *secondCutter;
};
...
int main(int argc, char *argv[])
{
    ...
    // 创建新的切平面用于切割第二个渲染器中的平面
    vtkSmartPointer<vtkPlane> secondPlane = vtkSmartPointer<vtkPlane>::New();
    secondPlane->SetOrigin(0.0, 0.0, 0.0);
    secondPlane->SetNormal(1.0, 0.0, 0.0);

    // 创建第二个切割器
    vtkSmartPointer<vtkCutter> secondCutter = vtkSmartPointer<vtkCutter>::New();
    secondCutter->SetCutFunction(secondPlane);
    secondCutter->SetInputConnection(cutter->GetOutputPort());
    secondCutter->Update();

    // 创建第三个 vtkGenericOpenGLRenderWindow
    vtkSmartPointer<vtkGenericOpenGLRenderWindow> renderWindow3 =
        vtkSmartPointer<vtkGenericOpenGLRenderWindow>::New();

    // 创建第三个 QVTKOpenGLNativeWidget 并设置渲染窗口
    QVTKOpenGLNativeWidget* vtkWidget3 = new QVTKOpenGLNativeWidget(&mainWindow);
    vtkWidget3->setRenderWindow(renderWindow3);

    // 创建第三个渲染器并添加到渲染窗口
    vtkSmartPointer<vtkRenderer> renderer3 = vtkSmartPointer<vtkRenderer>::New();
    renderWindow3->AddRenderer(renderer3);
    renderer3->SetBackground(0.7, 0.7, 0.7); // 设置背景颜色

    // 创建切割线的映射器
    vtkSmartPointer<vtkPolyDataMapper> lineMapper = vtkSmartPointer<vtkPolyDataMapper>::New();
    lineMapper->SetInputConnection(secondCutter->GetOutputPort());
    lineMapper->SetScalarVisibility(1);
    ...
    // 将切割线添加到第三个渲染器
    renderer3->AddActor(lineActor);

    // 创建第二个 vtkPlaneWidget 用于交互
    vtkSmartPointer<vtkPlaneWidget> secondPlaneWidget = vtkSmartPointer<vtkPlaneWidget>::New();
    secondPlaneWidget->SetInteractor(renderWindow2->GetInteractor());
    secondPlaneWidget->SetPlaceFactor(1.25);
    secondPlaneWidget->PlaceWidget();
    secondPlane->GetOrigin(origin);
    secondPlane->GetNormal(normal);
    secondPlaneWidget->SetOrigin(origin);
    secondPlaneWidget->SetNormal(normal);
    secondPlaneWidget->SetEnabled(1);

    // 创建第二个回调函数
    vtkSmartPointer<SecondPlaneCallback> secondCallback = vtkSmartPointer<SecondPlaneCallback>::New();
    secondCallback->secondCutter = secondCutter;
    secondPlaneWidget->AddObserver(vtkCommand::InteractionEvent, secondCallback);

    // 设置布局
    QHBoxLayout* layout = new QHBoxLayout(&mainWindow);
    layout->addWidget(vtkWidget1);
    layout->addWidget(vtkWidget2);
    layout->addWidget(vtkWidget3);
