...
**Feature**: 002-icon-design | **Version**: 2.0.0 | **Source**: research.md 决策 1–6、data-model.md §2.1

> 本契约是图标**视觉风格的唯一可检查定义**。凡交付图标必须满足以下每一条；任一不满足即计为符合性清单不符合项。
>
> **修订记录**：
> - 2.0.0（2026-08-29）——风格由"实心填充"调整为**线性圆角描边**：主体以描边（round cap/join）表达、无填充（对齐 Material Icons Outlined 官方视觉，替换 58 枚功能图标，`specs/007-replace-icon-set`）。基础网格 16×16 → **24×24**（Material 原生），尺寸换算基准改为 24 网格等比缩放（16=24×2/3、32=24×4/3），描边宽度档位调整为 **16=1.5 / 24=2 / 32=2.5**（16px 档下限 ≥1.5px）。P-01~P-04 / T-01~T-07 / N-01 / N-02 / A-02~A-04 不变；A-01 措辞同步（既有 app-icon 实心图形不追溯）。
> - 1.1.0（2026-08-25）——风格由"线性描边"调整为"实心填充"：主体以 Token 亮色实心填充，内部细节以深色负形挖空（对齐 SVisual 拟物参考，解决 16px 档线条单薄、辨识度不足问题）。色板白名单 P-01/P-03/P-04 不变。

## 1. 风格（Style）

| 条款 | 要求 |
|---|---|
| S-01 | 图标采用**线性圆角描边风格**：主体以描边（`stroke-linecap="round"`、`stroke-linejoin="round"`）表达，默认 `FG_TEXT #D4D4D4`，无填充（`fill="none"`）；`fill` 仅允许 `none` 或 Token 白名单色（个别辅助元素） |
...
| G-02 | 尺寸档位为 `16 / 24 / 32`；按 24 网格等比缩放（16=24×2/3、32=24×4/3），保证视觉比例一致 |
...
| 条款 | 要求 |
|---|---|
| A-01 | 应用图标与功能按钮图标共享同一视觉语言（S-01/S-02 适用；既有 app-icon 实心填充图形不追溯） |
