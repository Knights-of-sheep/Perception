...
- [X] T009 [P] [US1] 在 `src/ui/MainWindow.cpp` 的 `createActions()`/`createMenus()` 添加"视图 → 新建子窗口"动作，复用 `icon-map.yaml` 现有 `view-multi-view` 图标（`action_icon_map` 接入）；动作槽构造 `create_window("plot_N")` 命令文本并提交至 `PythonConsole::executeCommand` 执行（命令文本回显、返回值打印，FR-027），不绕过命令层（FR-002）
- [X] T010 [US1] 实现 `MainWindow::createSubwindow(const QString& title)`：创建 `SubwindowView`、加入容器并立即重排（FR-001）；Python 命令桥与菜单触发的命令行最终均调用此入口（FR-002、SC-008）
