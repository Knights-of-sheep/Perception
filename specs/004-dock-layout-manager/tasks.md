...
- [X] T008 [P] [US1] 在 `src/ui/console/PythonConsole.cpp` 注入全局函数 `create_window`（复用 `_cpp_log` 的 `PyMethodDef` + `PyCFunction_New` 模式，`PythonConsole.cpp:115-134`），返回新建子窗口 id（`"Plot_" + 序号`；title 缺省/空白时 title = id；非 str 抛 TypeError）按 `contracts/python-create-window.md`；经 `std::function` 回调注册于 MainWindow（`setCreateWindowCallback`，返回 id）；同时实现命令执行入口 `executeCommand`（执行命令文本、在输出区回显命令、打印返回值 id，FR-027）供菜单入口复用——与 PyShell 手工输入同一执行路径（plan Structure Decision）
- [X] T009 [P] [US1] 在 `src/ui/MainWindow.cpp` 的 `createActions()`/`createMenus()` 添加"视图 → 新建子窗口"动作，复用 `icon-map.yaml` 现有 `view-multi-view` 图标（`action_icon_map` 接入）；动作槽构造无参 `create_window()` 命令文本并提交至 `PythonConsole::executeCommand` 执行（命令文本回显、返回值 id 打印，FR-027），不绕过命令层（FR-002）
- [X] T010 [US1] 实现 `MainWindow::createSubwindow(const QString& title)`：返回生成的窗口 id（`"Plot_" + 序号`；title 缺省/空白时标题 = id），创建 `SubwindowView`、加入容器并立即重排（FR-001）；Python 命令桥与菜单触发的命令行最终均调用此入口（FR-002、SC-008）
