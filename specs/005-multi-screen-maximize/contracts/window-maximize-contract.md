...
2. 最大化几何：目标屏幕的 `availableGeometry()`（设备无关像素 DIP；副屏负坐标场景使用该屏幕自身的虚拟桌面坐标，不做相对换算）
3. 填充：`ptMaxPosition = (avail.x(), avail.y())`；`ptMaxSize = (avail.width(), avail.height())`
4. 处理结果：返回 `true` 并置 `*result = 0`

**输出**: Windows 按 `MINMAXINFO` 将窗口铺满目标屏幕工作区（不遮挡任务栏）。

**违反后果**: 目标屏解析错误 → 窗口最大化到错误屏幕/屏幕外（即本功能修复的"窗口消失"缺陷）；`ptMaxSize` 缺失 → 无边框窗口可能覆盖任务栏。
