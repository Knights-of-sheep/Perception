...
//   - R6（2026-08-29）：ptMaxPosition 为相对目标屏幕左上角的偏移（Windows
//     MINMAXINFO 语义），非虚拟桌面绝对坐标——副屏在主屏右/左/上方时偏移为 0
//     而非 1920/-1920 等虚拟坐标；任务栏在屏幕顶部/左侧时偏移非零（如 (0,40)）。
// 仅依赖 QtCore 类型（QRect/QList/QPoint/QSize），无 GUI 平台依赖，红-绿 TDD。
#include "ui/window_geometry.h"

#include <cassert>
#include <cstdio>

using perception::ui::window_geometry::maximizeInfo;
...
void expectInfo(const QList<QRect>& geom, const QList<QRect>& avail, int index,
                const QPoint& expectedPos, const QSize& expectedSize) {
    const auto got = maximizeInfo(geom, avail, index);
    if (got.maxPosition != expectedPos || got.maxSize != expectedSize) {
        fprintf(stderr,
                "maximizeInfo expected pos=(%d,%d) size=%dx%d "
                "got pos=(%d,%d) size=%dx%d (index=%d)\n",
                expectedPos.x(), expectedPos.y(), expectedSize.width(), expectedSize.height(),
                got.maxPosition.x(), got.maxPosition.y(), got.maxSize.width(),
                got.maxSize.height(), index);
        assert(false);
    }
}
...
int main() {
    // ---- FR-004：单屏（窗口中心命中）----
    {
        const QList<QRect> geom{{0, 0, 1920, 1080}};  // 含任务栏的完整几何
        const QList<QRect> avail{{0, 0, 1920, 1000}}; // 工作区（底部任务栏）
        const QRect frame{100, 100, 800, 600};
        expectIndex(0, geom, frame);
        // 任务栏在底部：偏移仅 y 方向 0（avail.top == geom.top），尺寸为工作区
        expectInfo(geom, avail, 0, QPoint(0, 0), QSize(1920, 1000));
    }

    // ---- FR-001/002：双屏横排，副屏在右（正坐标）----
    // R6 回归：ptMaxPosition 为相对目标屏偏移，副屏（虚拟 x=1920）应为 (0,0)
    // 而非虚拟坐标 (1920,0)——后者会把窗口推到目标屏右侧屏外。
    {
        const QList<QRect> geom{{0, 0, 1920, 1080}, {1920, 0, 1920, 1080}};
        const QList<QRect> avail{{0, 0, 1920, 1000}, {1920, 0, 1920, 1000}};
        // 窗口中心落在副屏 → 目标副屏：pos 相对副屏 = (0,0)
        const QRect frame{2000, 100, 800, 600};
        expectIndex(1, geom, frame);
        expectInfo(geom, avail, 1, QPoint(0, 0), QSize(1920, 1000));
        // 窗口中心仍落在主屏 → 目标主屏
    ...
    // ---- FR-001/002：副屏在左（负坐标）----
    // R6 回归：旧实现填入虚拟坐标 (-1920,0) 会把窗口推到屏外；现应相对副屏 (0,0)
    {
        const QList<QRect> geom{{-1920, 0, 1920, 1080}, {0, 0, 1920, 1080}};
    ...
    // ---- FR-001/002：竖排，副屏在上（负 y 坐标）----
    {
        ...
        expectIndex(0, geom, frame);
        expectInfo(geom, avail, 0, QPoint(0, 0), QSize(1920, 1040));
    }
    ...
    // ---- FR-001/002：不对称多屏（不同分辨率混合）----
    {
        ...
        expectIndex(1, geom, frame);
        expectInfo(geom, avail, 1, QPoint(0, 0), QSize(2560, 1400));
    }
    ...
    // ---- R6：任务栏在屏幕顶部 → 相对偏移非零（avail.top > geom.top）----
    {
        const QList<QRect> geom{{0, 0, 1920, 1080}, {1920, 0, 1920, 1080}};
        const QList<QRect> avail{{0, 40, 1920, 1040}, {1920, 40, 1920, 1040}};  // 顶部任务栏
        expectInfo(geom, avail, 1, QPoint(0, 40), QSize(1920, 1040));
        expectInfo(geom, avail, 0, QPoint(0, 40), QSize(1920, 1040));
    }

    // ---- R6：任务栏在屏幕左侧（竖排任务栏）→ 相对偏移非零 ----
    {
        const QList<QRect> geom{{0, 0, 1920, 1080}};
        const QList<QRect> avail{{60, 0, 1860, 1080}};
        expectInfo(geom, avail, 0, QPoint(60, 0), QSize(1860, 1080));
    }

    // ---- FR-005/006：窗口中心未命中任何屏幕（中心落在屏幕间缝隙）→ 回退首屏 ----
    {
        const QList<QRect> geom{{0, 0, 1920, 1080}, {2000, 0, 1920, 1080}};  // 缝隙 80px
    ...
    {
        const QList<QRect> geom{{0, 0, 1920, 1080}};  // 副屏已断开
    ...
    // 屏A 1920x1080 @100%（逻辑 1920x1080）；屏B 2560x1440 @125%（逻辑 2048x1152）
    {
        ...
        expectIndex(1, geom, frame);
        expectInfo(geom, avail, 1, QPoint(0, 0), QSize(2048, 1112));
        // 主屏窗口 → 主屏工作区
    ...
    // ---- 空列表/空项边界 ----
    {
        ...
        // 全空 → 空 MaximizeInfo（调用方保留原行为）
        const QList<QRect> emptyAvail;
        const auto emptyInfo = maximizeInfo(emptyGeom, emptyAvail, 0);
        if (!emptyInfo.isEmpty()) {
            fprintf(stderr, "maximizeInfo expected empty on empty lists\n");
            assert(false);
        }
        // index 越界 → 回退首个非空对
        const QList<QRect> geom{{0, 0, 1920, 1080}, {0, 0, 0, 0}};
        const QList<QRect> avail{{0, 0, 1920, 1000}, {0, 0, 0, 0}};
        expectInfo(geom, avail, 5, QPoint(0, 0), QSize(1920, 1000));
